...
namespace lz77 {
	...
	bool static delete_Node(Node* head, Node* to_delete) {
		if (to_delete == NULL)
			return false;
	...
	vector <string> static split(string str, char delimiter) {
...
-------------------------------------------------------------------------------
 LZ-77 >  Decompression :)";
	...
	static string LZ77(string input, int option) {
			...
			for (int j = 0; j < length; j++) {
				...
				if (result_ary[0][j] == 0 && result_ary[1][j] == 0) {
					if (result_ary[2][j] != NULL || result_ary[2][j] != 0) {
						...
						ss << to_string(result_ary[0][j]);
						ss << ",";
						ss << to_string(result_ary[1][j]);
						ss << ",";
						ss << z;
						ss << " ";
					}
					...
					//char z = result_ary[2][j];
					ss << to_string(result_ary[0][j]) << "," << to_string(result_ary[1][j]) << ",0 ";
				}
				result += ss.str();
			}
	...
	static string LZ78(string input, int option) {
		if (option == 1) {
	...
	static string LZW(string input, int option) {
		if (option == 1) {
			Node* dictionary = new Node;
			string result;
