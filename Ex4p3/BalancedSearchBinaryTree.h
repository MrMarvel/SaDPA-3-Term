...
public:
    ...
    inline shared_ptr<T> searchValueByKey(K key) {
...
template<typename K, typename T>
void BalancedSearchBinaryTree<K, T>::displayBinTree(bool show) {
    printBinTree(root, show);
}

template<typename K, typename T>
void BalancedSearchBinaryTree<K, T>::printBinTree(TreeNode* node, bool show, long n) {
    if (!node) return;
